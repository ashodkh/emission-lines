...
for N in Ns:
    ...
    x, y, ivar, EW = features_and_outcomes_classification(fluxes_bin, target_lines, line_ivars, int(n_train), 5, 6)
    x_test, y_test, ivar_test, EW_test = features_and_outcomes_classification(fluxes_bin_test, target_lines_test, line_ivars_test, int(n_test), 5, 6)

    # Predicting classification. The result is a number between (roughly) 0 and 1, with a threshold that selects yes and no. True positive rate is not very sensitive to this threshold
    # but false positive rate is, so I chose 0.7 threshold, which gives false positive rate ~27%
    ...
    for m in m_j:
        ...
        np.savez_compressed(server_paths[server] + "ew_results_classification/" + data_file_names[data] + "/m" + str(m) + "/classification_test_logEW_fit_" + data_file_names[data] + "_selection" + str(run) + \
                                    "_line" + str(lines[l]) + "_bins" + str(N) + "_ML" + str(m) + ".txt", EW_fit_all)
        np.savez_compressed(server_paths[server] + "ew_results_classification/" + data_file_names[data] + "/m" + str(m) + "/classification_test_EW_obs_" + data_file_names[data] + "_selection" + str(run) + \
                                    "_line" + str(lines[l]) + "_bins" + str(N) + "_ML" + str(m) + ".txt", EW_test_all)
        np.savez_compressed(server_paths[server] + "ew_results_classification/" + data_file_names[data] + "/m" + str(m) + "/classification_test_line_ivars_" + data_file_names[data] + "_selection" + str(run) + \
                                    "_line" + str(lines[l]) + "_bins" + str(N) + "_ML" + str(m) + ".txt", ivar_test_all)